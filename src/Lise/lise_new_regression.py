...
data_file = "Veriseti_Anadolu_Liseleri_GONDERILEN.xlsx"
...
girdiler = [1,3,0,375.0,62,56]
data_girdi=pd.DataFrame(columns=['Asag','internet','okul_dyk','lgs_puani','turkce9','mat9'])
data_girdi = data_girdi.append({'Asag': girdiler[0],
                                'internet': girdiler[1],
                                'okul_dyk': girdiler[2],
                                'lgs_puani': girdiler[3],
                                'turkce9': girdiler[4],
                                'mat9': girdiler[5]},
                                ignore_index=True)
...
"""`internet, Boz, sosyal_kulturel` değişkenlerinin p-value'su 0.05'den büyük olduğu için değişkenler arasından çıkarmamız gerekir.
...
coefs = model.params
coefs = pd.DataFrame(coefs, columns=["coefficients"])
coefs.drop(index=["const","internet", "Boz", "sosyal_kulturel"], inplace=True)
coefs
...
girdiler = [0,4,0,0,375, 67]
data_girdi=pd.DataFrame(columns=['cinsiyet','ders_calisma','okul_dyk','ortaokul_turu',"lgs_puani",'ort9'])
data_girdi = data_girdi.append({'cinsiyet': girdiler[0],
                                'ders_calisma': girdiler[1],
                                'okul_dyk': girdiler[2],
                                'ortaokul_turu': girdiler[3],
...
"""`okul_dyk` değişkenlerinin p-value'su 0.05'den büyük olduğu için değişkenler arasından çıkarmamız gerekir.
...
coefs = model.params
coefs = pd.DataFrame(coefs, columns=["coefficients"])
coefs.drop(index=["const", "okul_dyk"], inplace=True)
coefs
...
x = df[["cinsiyet","ABayri","ders_calisma","sosyal_kulturel","turkce9","ort10"]]
#x = df[ort11_futures]
...
girdiler = [0,0,3,0,88,91]
data_girdi=pd.DataFrame(columns=["cinsiyet","ABayri","ders_calisma","sosyal_kulturel","turkce9","ort10"])
data_girdi = data_girdi.append({'cinsiyet': girdiler[0],
                                'ABayri': girdiler[1],
                                'ders_calisma': girdiler[2],
                                'sosyal_kulturel': girdiler[3],
                                'turkce9': girdiler[4],
                                'ort10': girdiler[5]},
                                ignore_index=True)
